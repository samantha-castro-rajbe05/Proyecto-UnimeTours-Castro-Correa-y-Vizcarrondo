...
const Signup = () => {
  ...
  const handleTelefonoChange = (e) => {
    ...
    if (regex.test(value)) {
      ...
      setTelefonoError("El número de teléfono debe comenzar con +58 seguido de 414, 412, 424, 212, 416, o 426 y contener 7 dígitos después del guion.");
    }
  };
        ...
        <div>
          <form onSubmit={handleSignup} className="max-w-5xl mx-auto pt-10 px-10 lg:px-0">
            ...
            <div className="border-b border[#143A27]/20 pb-5">
              ...
              <button type="submit" className="mt-5 bg-[#143A27] w-full py-2 uppercase text-base font-bold tracking-wide rounded-md text-[#D4D9D8] hover:text-white hover:bg-[#02321A] duration-200">
                Registrarse
              </button>
            </div>
          </form>
          <p className="text-sm leading-6 text-[#4B6C64] text-center py-6">
            ¿Ya tienes una cuenta creada? <button onClick={() => setLogin(true)} className="text-[#143A27] font-semibold underline underline-offset-2 decoration-[1px] hover:text-white duration-200">Iniciar sesión</button>
          </p>
        </div>
      )}
    </div>
